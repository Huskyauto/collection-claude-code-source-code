...
import { Plus, MessageSquare, Settings, Zap, Trash2, Bot, Brain, Users, Heart, BookOpen, Search, X, CreditCard, BarChart3, Download, Loader2, Home, LogOut, LogIn, Mail, FolderOpen, Library, Phone, KeyRound, CalendarClock, ChevronDown, Wrench, Shield, FlaskConical, Lightbulb, Radio, Rocket, Send } from "lucide-react";
import {
  ...
  return (
    <Sidebar className="border-r border-sidebar-border">
      ...
      <SidebarFooter className="border-t border-sidebar-border p-2 max-h-[40vh] overflow-y-auto">
        ...
        {(isAdmin || isPaid) && (
          <NavSection title="Tools" icon={Wrench} defaultOpen={["/skills", "/skills-marketplace", "/personality-files", "/analytics", "/email", "/whatsapp", "/whatsapp-approval", "/telegram", "/mcp", "/webhook-triggers", "/channel-routing"].some(p => location === p)}>
            ...
            {(isAdmin || isPaid) && <NavLink path="/analytics" icon={BarChart3} label="Analytics" />}
            {(isAdmin || isPaid) && <NavLink path="/email" icon={Mail} label="Email" />}
            {isAdmin && <NavLink path="/whatsapp" icon={Phone} label="WhatsApp" />}
            <NavLink path="/whatsapp-approval" icon={Phone} label="WhatsApp Approvals" />
            <NavLink path="/telegram" icon={Send} label="Telegram" />
        ...
        {isAdmin && (
          <NavSection title="Admin" icon={Shield} defaultOpen={["/heartbeat", "/agentic", "/payments", "/settings"].some(p => location === p)}>
            <NavLink path="/heartbeat" icon={Heart} label="Heartbeat Engine" />
            <NavLink path="/agentic" icon={Radio} label="Agentic Ops" />
            <NavLink path="/payments" icon={CreditCard} label="Payments" />
