...
async function getOrCreateSharedBrowser(profileName: string): Promise<Browser> {
  ...
  try {
    ...
    console.log(`[browser] Connected to "${name}" via ${isWebSocket ? "WebSocket" : "HTTP"} CDP`);
    return sharedBrowser;
  } catch (err: any) {
    throw new Error(`Could not connect to browser service at the configured URL. Check your browser profile settings and ensure the service is running.`);
  }
}
...
async function uploadScreenshotToDrive(screenshotBuffer: Buffer, filename: string, pageTitle: string): Promise<{ driveUrl?: string; directDownloadUrl?: string }> {
  try {
...
export async function takeScreenshot(index?: number, fullPage?: boolean, selector?: string, profileName?: string, tenantId?: number, returnBase64?: boolean): Promise<any> {
  ...
  const screenshotUrl = driveResult.driveUrl || localScreenshotUrl;

  const result: any = {
    success: true,
    screenshotUrl,
    screenshotMarkdown: `![Screenshot of ${pageTitle}](${screenshotUrl})`,
  ...
  try {
    switch (params.action) {
      ...
      case "smart_browse": {
        ...
        emitBrowserLive(tenantId, "screenshot", `Captured: ${title}`, { screenshotUrl: localSmartUrl, pageTitle: title, pageUrl: url });
        emitBrowserLive(tenantId, "analyzing", `Analyzing page content...`, { pageTitle: title, pageUrl: url });

        const smartDriveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, title);
        const smartScreenshotUrl = smartDriveResult.driveUrl || localSmartUrl;
        ...
        const smartResult: any = {
          ...
          url,
          title,
          screenshotUrl: smartScreenshotUrl,
          screenshotMarkdown: `![Screenshot of ${title}](${smartScreenshotUrl})`,
...
export async function injectSomAndScreenshot(
  ...
  emitBrowserLive(tenantId, "screenshot", `Vision scan: ${somResult.pageTitle}`, { screenshotUrl: localSomUrl, pageTitle: somResult.pageTitle, pageUrl: somResult.pageUrl || "" });

  const driveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, somResult.pageTitle);
  const screenshotUrl = driveResult.driveUrl || localSomUrl;
  ...
  const result: any = {
    ...
    scrollContext,
    screenshotUrl,
    screenshotMarkdown: `![SoM annotated screenshot of ${somResult.pageTitle}](${screenshotUrl})`,
    elementSummary,
