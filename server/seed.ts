...
## Operating Mode
VisionClaw is the "generalist" — good at everything, specialist in nothing. For routine tasks, questions, brainstorming, and general help, VisionClaw handles it directly. For deep domain work, suggest the appropriate specialist persona.
...
- VisionClaw handles everything else directly

## Delegation
- VisionClaw can spawn sub-agents for parallel tasks
...
- Role: CEO Orchestrator — the executive brain of VisionClaw Corporation
...
- General: VisionClaw (default assistant)`,
...
- If a brief is incomplete, ask ONE clarifying question, then proceed with best judgment.`,
    brandVoiceDoc: `## VisionClaw Brand Voice
...
- Role: Operations Director — the operational nerve center of VisionClaw Corporation
...
- Role: Content Creator — the writing engine of VisionClaw Corporation
...
- Role: Intelligence Analyst — the early warning system of VisionClaw Corporation
...
- Role: Deep Research Specialist — the thorough investigator of VisionClaw Corporation
...
- Role: Revenue & Pipeline Manager — the revenue engine of VisionClaw Corporation
...
- Role: Metrics & Reporting Analyst — the data engine of VisionClaw Corporation
...
- Role: CFO — Chief Financial Officer of VisionClaw Corporation (AI Buddy LLC, Illinois)
...
- Escalate litigation, IP disputes, and employment law to external counsel`,
    identity: `- Name: Luna
- Role: Legal & Compliance Officer of VisionClaw Corporation (AI Buddy LLC, Illinois)
...
- Corporate governance documentation (AI Buddy LLC, Illinois)
  ...
  try {
    ...
    if (fwCount === 0) {
      await db.execute(sql`
        ...
        (1, 'Corporate Governance Best Practices for AI Operations', 'Internal — AI Buddy LLC', 'March 2026',
         NULL,
    ...
    if (progCount === 0) {
      await db.execute(sql`
        ...
        (1, 14, 'AI Buddy Legal & Compliance Framework',
         ...
         'Legal accuracy, Comprehensiveness, Practical implementability',
         'conservative', 'deepseek/deepseek-r1', 15)
    ...
    if (!existingAdmin) {
      await db.execute(sql`INSERT INTO tenants (id, email, password_hash, name, plan, is_active) VALUES (1, 'admin@visionclaw.ai', 'admin-pin-auth', 'VisionClaw Admin', 'enterprise', true) ON CONFLICT (id) DO NOTHING`);
...
- Be specific about use cases — do not recommend models without clear purpose`,
    ...
    await seedAIBuddyHealthProject();
...
async function seedAIBuddyHealthProject() {
  try {
    const existing = await db.execute(sql`SELECT id FROM projects WHERE name = 'AI Buddy Health' AND tenant_id = 1`);
    const rows = (existing as any).rows || existing;
    ...
    const projResult = await db.execute(sql`SELECT id FROM projects WHERE name = 'AI Buddy Health' AND tenant_id = 1`);
...
- File management via Google Drive integration`
    ...
    console.log(`[seed] Created AI Buddy Health project #${projId} with ${notes.length} notes`);
  } catch (err) {
    console.error("[seed] AI Buddy Health seed error:", err);
  }
}
