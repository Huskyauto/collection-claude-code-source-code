import { sendEmail, getOrCreateTenantInbox, getOrCreatePrimaryInbox, isEmailConfigured } from "./email";

const FROM_NAME = "VisionClaw Agent";
...
<p style="color:#888;margin:4px 0 0;font-size:13px;">Agentic AI Corporation</p>
</div>
<div style="background:#1a1a1a;border-radius:12px;padding:30px;border:1px solid #333;">
${body}
</div>
<div style="text-align:center;margin-top:30px;color:#666;font-size:12px;">
<p>&copy; ${new Date().getFullYear()} AI Buddy LLC. All rights reserved.</p>
<p style="margin-top:8px;">
<a href="https://visionclawagent.com/terms" style="color:#888;text-decoration:none;">Terms</a> &middot;
<a href="https://visionclawagent.com/privacy" style="color:#888;text-decoration:none;">Privacy</a>
</p>
</div>
</div>
</body>
...
export async function sendVerificationEmail(tenantEmail: string, code: string): Promise<boolean> {
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#ccc;line-height:1.6;">Welcome to VisionClaw! Enter this code to verify your email address:</p>
    ...
    const text = `Your VisionClaw verification code is: ${code}\n\nThis code expires in 15 minutes.`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `${code} — Your VisionClaw verification code`,
      text,
      html,
    });
...
export async function sendWelcomeEmail(tenantEmail: string, tenantName: string): Promise<boolean> {
  ...
  try {
    ...
    const html = wrapHtml(`
      <h2 style="color:#fff;margin:0 0 15px;font-size:20px;">Welcome to VisionClaw Agent!</h2>
      <p style="color:#ccc;line-height:1.6;">Hi ${tenantName},</p>
      <p style="color:#ccc;line-height:1.6;">You now have access to your own AI-powered agent team — 12 specialized personas ready to help you with everything from research and content creation to code generation and business analytics.</p>
      ...
      <ul style="color:#ccc;line-height:1.8;padding-left:20px;">
        ...
        <li><strong style="color:#fff;">Explore personas</strong> — Meet Forge (code), Teagan (content), Radar (research), and more</li>
      ...
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Launch VisionClaw</a>
      </div>
    `);

    const text = `Welcome to VisionClaw Agent!\n\nHi ${tenantName},\n\nYou now have access to your own AI-powered agent team. Start a conversation at https://visionclawagent.com\n\nYour free trial includes 5 conversations.\n\n- The VisionClaw Team`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `Welcome to VisionClaw Agent, ${tenantName}!`,
      text,
      html,
    });
...
export async function sendPasswordResetEmail(tenantEmail: string, tenantName: string, resetToken: string, baseUrl?: string): Promise<boolean> {
  ...
  try {
    ...
    const base = baseUrl || "https://visionclawagent.com";
    ...
    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: "VisionClaw Agent — Password Reset",
      text,
      html,
    });
...
export async function sendUsageWarningEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com/?upgrade=true" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Upgrade Plan</a>
      </div>
    `);

    const text = `Usage Alert: ${pct}% of ${metricLabel} used\n\nYou've used ${current} of ${limit} ${metricLabel} on your ${plan} plan.\n\nUpgrade at https://visionclawagent.com/?upgrade=true`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `VisionClaw Usage Alert: ${pct}% of ${metricLabel} used`,
      text,
      html,
    });
...
export async function sendPlanUpgradeEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#ccc;line-height:1.6;">Thank you for choosing VisionClaw Agent. Your AI team is ready to work harder for you.</p>
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Open VisionClaw</a>
      </div>
    `);
    ...
    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `Plan Upgraded: VisionClaw ${newPlan.charAt(0).toUpperCase() + newPlan.slice(1)}`,
      text,
      html,
    });
...
export async function sendLimitReachedEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com/?upgrade=true" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Upgrade Now</a>
      </div>
    `);

    const text = `Limit Reached: ${metricLabel}\n\nYou've hit your ${limit} ${metricLabel} limit on the ${plan} plan. Resets at ${resetTime}.\n\nUpgrade: https://visionclawagent.com/?upgrade=true`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `VisionClaw: ${metricLabel} limit reached`,
      text,
      html,
    });
...
export async function sendPaymentFailedEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#ccc;line-height:1.6;">Hi ${tenantName},</p>
      <p style="color:#ccc;line-height:1.6;">We were unable to process your latest payment for your VisionClaw Agent subscription. This could be due to an expired card, insufficient funds, or a temporary issue with your payment method.</p>
      ...
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com/settings" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Update Payment Method</a>
      </div>
    `);

    const text = `Payment Failed\n\nHi ${tenantName},\n\nWe couldn't process your latest payment. Please update your payment method at https://visionclawagent.com/settings to avoid service interruptions.\n\n- The VisionClaw Team`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `Action Required: VisionClaw Payment Failed`,
      text,
      html,
    });
...
export async function sendSubscriptionCancelledEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#ccc;line-height:1.6;">Hi ${tenantName},</p>
      <p style="color:#ccc;line-height:1.6;">Your VisionClaw Agent subscription has been cancelled. Your account has been moved to the free tier.</p>
      ...
      <div style="text-align:center;margin-top:25px;">
        <a href="https://visionclawagent.com" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:600;">Resubscribe</a>
      </div>
    `);

    const text = `Subscription Cancelled\n\nHi ${tenantName},\n\nYour subscription has been cancelled and your account moved to the free tier. You can resubscribe anytime at https://visionclawagent.com\n\n- The VisionClaw Team`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `VisionClaw: Subscription Cancelled`,
      text,
      html,
    });
...
export async function sendAccountDeletionScheduledEmail(
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#ccc;line-height:1.6;">Hi ${tenantName},</p>
      <p style="color:#ccc;line-height:1.6;">Your VisionClaw Agent account has been scheduled for permanent deletion on <strong style="color:#fff;">${dateStr}</strong>.</p>
      ...
      <div style="text-align:center;margin-top:25px;display:flex;gap:12px;justify-content:center;">
        <a href="https://visionclawagent.com/settings" style="display:inline-block;background:#333;color:#fff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:600;">Cancel Deletion</a>
        <a href="https://visionclawagent.com/files" style="display:inline-block;background:${BRAND_COLOR};color:#fff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:600;">Download My Files</a>
      </div>
    `);

    const text = `Account Deletion Scheduled\n\nHi ${tenantName},\n\nYour account is scheduled for permanent deletion on ${dateStr}.\n\nBefore that date, please:\n- Download your files from the Files page\n- Export your data from Settings > Data\n\nAll data will be permanently deleted after ${dateStr}.\n\nTo cancel, visit https://visionclawagent.com/settings\n\n- The VisionClaw Team`;

    await sendEmail({
      inboxId,
      to: tenantEmail,
      subject: `VisionClaw: Account Deletion Scheduled for ${dateStr}`,
      text,
      html,
    });
  ...
  try {
    ...
    const html = wrapHtml(`
      ...
      <p style="color:#888;font-size:12px;margin-top:16px;">
        ...
        <strong>App issues</strong> = VisionClaw code problem. <strong>Infrastructure</strong> = Replit/hosting issue. <strong>Integration</strong> = third-party service.
      </p>
    `);
