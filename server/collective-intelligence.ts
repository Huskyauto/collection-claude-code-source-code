...
export async function canUseProtocol(tenantId: number, protocol: ProtocolType): Promise<{ allowed: boolean; reason?: string }> {
  ...
  try { const { getProtocolLimit } = require("./auto-tuner"); limits = { chain_of_debates: getProtocolLimit("chain_of_debates"), tree_of_thought: getProtocolLimit("tree_of_thought"), full_council: getProtocolLimit("full_council") }; } catch {}
  ...
  try {
    ...
    if (spend.metrics.spend_percent >= 80 && protocol !== "specialist_critique") {
