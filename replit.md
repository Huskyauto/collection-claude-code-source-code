...
- **95 AI Tools:** Comprehensive toolset for communication, research, documents, code execution, virtual browsing, web scraping, agentic operations, Google Workspace, and system management, including `generate_dashboard` for Live Canvas, `strategic_interview` for Socratic requirement gathering, `export_persona` for portable agent definitions, `render_diagram` for Mermaid-to-image diagram generation, `vibevoice_transcribe`/`vibevoice_speak` for Microsoft VibeVoice frontier voice AI, and `trend_research` for multi-source trend analysis (Reddit, HN, Polymarket, X — inspired by /last30days).
...
- **Microsoft VibeVoice Integration:** Frontier open-source voice AI from Microsoft, integrated as tools #93-94. Three model tiers: (1) **VibeVoice ASR** (`vibevoice_transcribe`) — 60-minute single-pass speech-to-text with speaker diarization (who/when/what), 50+ languages, custom hotwords, via HF Inference API with Gradio fallback; (2) **VibeVoice TTS** (`vibevoice_speak`) — 90-minute multi-speaker text-to-speech with up to 4 expressive voices for podcast/dialogue generation; (3) **VibeVoice Realtime** — 0.5B streaming TTS with ~200ms latency. Also wired as a TTS provider option in the voice system alongside ElevenLabs, OpenAI, and Google Translate. Available speakers: Carter, Alyssa, Angelo, Bella, Davis, Elijah, Evelyn, James, Joanna, Kenji, Madeline, Nova. API endpoints: `/api/vibevoice/info`, `/api/vibevoice/transcribe`, `/api/vibevoice/speak`.
