...
- **Finance Market Intelligence Tools:** `server/finance-tools.ts` — 4 tools adapted from Awesome-finance-skills (Apache 2.0). `finance_news` fetches real-time headlines from 10+ sources (Cailian Press, WallStreetCN, Xueqiu, Hacker News, Weibo, etc.) via NewsNow API. `finance_stock_price` retrieves OHLCV data from EastMoney Direct (A-Share & HK markets, no API key needed). `finance_stock_search` finds tickers by company name or code. `finance_market_overview` returns major index snapshots. All free, no auth required. Mapped to Cassandra (primary) and Radar via tool router, with finance keywords in scaffolding triggers.
- **87+ AI Tools:** Comprehensive toolset for communication, research, documents, code execution, virtual browsing, web scraping (FireCrawl scrape/crawl/map with database storage), agentic operations, Google Workspace, and system management, including `generate_dashboard` for Live Canvas.
