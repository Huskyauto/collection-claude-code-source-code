...
- **94 AI Tools:** Comprehensive toolset for communication, research, documents, code execution, virtual browsing, web scraping, agentic operations, Google Workspace, and system management, including `generate_dashboard` for Live Canvas, `strategic_interview` for Socratic requirement gathering, `export_persona` for portable agent definitions, `render_diagram` for Mermaid-to-image diagram generation, and `vibevoice_transcribe`/`vibevoice_speak` for Microsoft VibeVoice frontier voice AI.
